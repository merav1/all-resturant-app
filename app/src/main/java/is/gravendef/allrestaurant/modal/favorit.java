...
public class favorit {
    ...
    public favorit(String url,String nameM,String nameP,String nameQ,boolean b) {
        this.url = url;
        this.nameM = nameM;
        this.nameP = nameP;
        this.nameQ = nameQ;
    ...
    public static void setLastrestoId(int lastrestoId) {
        favorit.lastrestoId = lastrestoId;
    }

    private static int lastrestoId = 0;
    public static ArrayList<favorit> createPanierList(int numResto) {
        ArrayList<favorit> favorits = new ArrayList<favorit>();

        for (int i = 1; i <= numResto; i++) {
            favorits.add(new favorit("Person " + ++lastrestoId,"name","price","quantity", i <= numResto / 2));
        }
